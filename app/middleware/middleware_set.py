...
async def banned_ips_middleware(request: Request, call_next: Callable):
    ...
    по предопределенному списку запрещенных IP-адресов. Если IP-адрес недействителен или находится
    в запрещенном списке, возвращается ответ 403 Forbidden. В противном случае запрос передается следующему middleware или конечной точке.

    Args:
        request (Request): входящий HTTP-запрос.
        call_next (Callable): следующее middleware или конечная точка для вызова.
    Returns:
        JSONResponse: ответ 403, если IP-адрес запрещен или недействителен, в противном случае
    """
    try:
